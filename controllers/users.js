    ...
    .then((user) => {
      ...
      res
        .cookie('jwt', token, {
          ...
          sameSite: 'none', // отдаёт куки только родному домену, если браузер поддерживает эту фичу. Для кроссдоменных запросов в chrome нужна настройка 'none', однако когда фронт будет на домене сервера, нужно поставить true (! не строка)
          secure: true,
        })
